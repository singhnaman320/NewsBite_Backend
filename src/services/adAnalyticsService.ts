...
export const getAdAnalytics = async () => {
  ...
  const viewMap = new Map(
    viewAggregation.map((item) => [String(item._id), item.uniqueViews]),
  );
  const clickMap = new Map(
    clickAggregation.map((item) => [String(item._id), item.totalClicks]),
  );
  ...
  return ads.map((ad) => {
    ...
    return {
      ...
      ctr:
        uniqueViews === 0
          ? 0
          : Number(((totalClicks / uniqueViews) * 100).toFixed(2)),
    };
  });
};
